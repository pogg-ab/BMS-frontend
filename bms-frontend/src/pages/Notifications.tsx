    ...
    <PageLayout
      ...
      actions={
        <div className="flex bg-slate-100 dark:bg-slate-900 p-1 rounded-xl">
          ...
          >
            <Bell size={16} /> Inbox {localUnread > 0 && <span className="w-5 h-5 rounded-full bg-rose-500 text-white text-[10px] flex items-center justify-center animate-pulse">{localUnread}</span>}
          </button>
          <button
            onClick={() => setTab('announce')}
            className={`px-4 py-2 text-sm font-bold rounded-lg transition-all flex items-center gap-2 ${tab === 'announce' ? 'bg-white dark:bg-slate-800 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl pl-11 pr-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 transition-all font-medium"
                />
              </div>
              <div className="flex gap-2 items-center">
                <button
                  onClick={() => setShowUnreadOnly(!showUnreadOnly)}
                  className={`px-3 py-2.5 text-xs font-bold rounded-xl border transition-all ${showUnreadOnly ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white dark:bg-slate-800 text-slate-500 border-slate-200 dark:border-slate-700 hover:border-indigo-300'}`}
                >
                  Unread Only
                </button>
                <button
                  ...
                  className="px-3 py-2.5 text-xs font-bold text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-xl transition-all disabled:opacity-30 flex items-center gap-1.5 border border-slate-200 dark:border-slate-700"
                >
              ...
              {TYPES.map(t => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider rounded-lg transition-all ${isActive ? `${meta.bg} ${meta.color} ring-2 ring-offset-1 ring-indigo-400` : 'bg-slate-50 dark:bg-slate-800/60 text-slate-400 hover:text-slate-600'}`}
                  >
                    {t}
                  </button>
                )
