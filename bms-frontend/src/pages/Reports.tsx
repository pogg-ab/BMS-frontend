...
function Money({ value }: { value: number }) {
  return <span className="font-mono">ETB {Number(value).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
}
  ...
  return (
    <PageLayout title="Reports" subtitle="Occupancy, financials and dashboard KPIs">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 rounded shadow p-6 lg:col-span-2">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow-sm p-6 lg:col-span-1">
            ...
            Array.isArray(occupancy) ? (
              <div className="space-y-3">
                ...
                <div className="divide-y rounded border overflow-hidden">
                  {occupancy.map((l:any) => (
                    <div key={l.id} className="p-4 flex gap-4 items-start hover:bg-gray-50 dark:bg-slate-900">
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between">
                          <div>
                            ...
                            <div className="text-xs text-gray-500 mt-1">Floor {l.unit?.floor ?? '-'} · {l.unit?.size_sqm ?? '-'} sqm · Rent: <span className="font-medium text-slate-900 dark:text-white">ETB {Number(l.unit?.rent_price || 0).toLocaleString()}</span></div>
                          </div>
                        ...
                        <div className="mt-3 flex items-center justify-between text-xs text-gray-400 font-mono">
                          <div>{new Date(l.start_date).toLocaleDateString()}{l.end_date ? ` → ${new Date(l.end_date).toLocaleDateString()}` : ''}</div>
                        </div>
                      </div>
                    </div>
                  ))}
