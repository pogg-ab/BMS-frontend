...
export default function NavBar() {
  ...
  const { sidebarCollapsed, toggleSidebar, isMobileSidebarOpen, setIsMobileSidebarOpen } = useLayout()
  ...
  function isItemVisible(item: NavItem) {
    ...
    if (item.permissions) {
      if (item.permissions.some((p: string) => userPermissions.includes(p))) return true
    }
    if (item.permission) {
      ...
      >
        ...
        <div className="h-[60px] flex items-center justify-between px-4 border-b border-slate-200 dark:border-slate-800">
          ...
          <button 
            ...
            className="hidden lg:block p-1.5 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors shrink-0"
                  ...
                  {visibleItems.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={`whitespace-nowrap transition-all duration-300 ${sidebarCollapsed ? 'w-0 opacity-0 overflow-hidden lg:w-0' : 'w-auto opacity-100'}`}>
